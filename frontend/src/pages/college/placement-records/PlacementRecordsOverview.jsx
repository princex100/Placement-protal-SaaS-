...
// Dummy Fallback Data
const DUMMY_RECORDS = [
  { _id: "dummy1", branch: "Computer Science (CSE)", totalStudentsInBranch: 120, eligibleStudents: 100, placedStudentsCount: 85 },
  { _id: "dummy2", branch: "Information Technology (IT)", totalStudentsInBranch: 90, eligibleStudents: 80, placedStudentsCount: 70 },
  { _id: "dummy3", branch: "Electronics (ECE)", totalStudentsInBranch: 110, eligibleStudents: 95, placedStudentsCount: 60 },
];
    ...
    const fetchRecords = async () => {
      try {
        ...
        if (Array.isArray(data) && data.length > 0) {
          ...
          // If no records exist yet, use dummy to show UI
          setRecords(DUMMY_RECORDS);
        }
      } catch (error) {
        console.warn("Backend not ready or failed, using dummy data:", error);
        setRecords(DUMMY_RECORDS);
      } finally {
        setLoading(false);
      }
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        {records.map((record) => (
          ...
          >
            ...
            <h3 className="mb-4 text-xl font-bold text-slate-900 dark:text-white">{record.branch}</h3>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-semibold text-slate-900 dark:text-white">{record.totalStudentsInBranch}</span>
              </div>
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-bold text-emerald-600 dark:text-emerald-400">{record.placedStudentsCount}</span>
              </div>
            </div>
            ...
            <div className="mt-6 h-2 w-full overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
              <div 
                ...
                style={{ width: `${record.eligibleStudents ? (record.placedStudentsCount / record.eligibleStudents) * 100 : 0}%` }} 
              />
            </div>
          </motion.div>
        ))}
