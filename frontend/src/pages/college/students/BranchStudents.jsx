...
const BranchStudents = () => {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      default: // unplaced
        return "bg-slate-200 dark:bg-white/[0.06] text-slate-500 dark:text-neutral-400";
    }
  };
      ...
      <div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
          <button 
            ...
            className="mb-4 flex items-center gap-2 text-sm font-medium text-slate-500 dark:text-neutral-400 transition hover:text-slate-900 dark:hover:text-white"
          >
            <ArrowLeft size={16} /> Back to Branches
          </button>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-slate-900 dark:text-white">Branch Students</h1>
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input 
              ...
              className="w-full rounded-xl border border-slate-200 dark:border-white/[0.06] bg-slate-100 dark:bg-white/[0.04] py-2.5 pl-10 pr-4 text-sm text-slate-900 dark:text-white placeholder:text-slate-700 dark:text-neutral-600 focus:border-indigo-500/50 focus:outline-none focus:ring-1 focus:ring-indigo-500/30 md:w-[200px]"
            />
          </div>
          <button className="flex h-[42px] items-center gap-2 rounded-xl border border-slate-200 dark:border-white/[0.06] bg-slate-100 dark:bg-white/[0.04] px-4 text-sm font-semibold text-neutral-300 transition hover:bg-slate-200 dark:hover:bg-white/[0.06]">
            <Filter size={16} />
          </button>
          <button
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-600 dark:text-neutral-500">{student.email || "No email provided"}</p>
                        </div>
                      </div>
                    </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4 py-8 backdrop-blur-sm overflow-y-auto">
          <div className="w-full max-w-xl rounded-2xl border border-slate-200 dark:border-white/[0.08] bg-white dark:bg-[#12121e] p-6 shadow-2xl shadow-black/40 my-auto">
            ...
            <form onSubmit={handleAddStudent} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="mb-1 block text-sm font-medium text-slate-500 dark:text-neutral-400">Roll Number *</label>
                <input type="text" name="rollNo" required value={formData.rollNo} onChange={handleInputChange} className="w-full rounded-xl border border-slate-200 dark:border-white/[0.06] bg-slate-100 dark:bg-white/[0.04] px-3 py-2 text-sm text-slate-900 dark:text-white placeholder:text-slate-700 dark:text-neutral-600 focus:border-indigo-500/50 focus:outline-none" />
              </div>
              <div>
                <label className="mb-1 block text-sm font-medium text-slate-500 dark:text-neutral-400">Password *</label>
                <input type="password" name="password" required minLength={6} value={formData.password} onChange={handleInputChange} className="w-full rounded-xl border border-slate-200 dark:border-white/[0.06] bg-slate-100 dark:bg-white/[0.04] px-3 py-2 text-sm text-slate-900 dark:text-white placeholder:text-slate-700 dark:text-neutral-600 focus:border-indigo-500/50 focus:outline-none" />
              </div>
              <div>
              ...
              <div className="md:col-span-2 mt-4 flex justify-end gap-3 border-t border-slate-200 dark:border-white/[0.06] pt-6">
                <button type="button" onClick={() => setIsModalOpen(false)} className="rounded-xl px-4 py-2 text-sm font-semibold text-slate-500 dark:text-neutral-400 transition hover:bg-slate-100 dark:hover:bg-white/[0.04]">
                  Cancel
                </button>
